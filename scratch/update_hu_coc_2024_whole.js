...
// 1. Remove the previously added split hu-2024-coc-* questions
const initialCount = data.length;
data = data.filter(q => !q.id.startsWith('hu-2024-coc-'));
console.log(`Removed ${initialCount - data.length} previously added split questions.`);

// 2. Add as one unified COC exam (course: "Freshman COC", category: "COC Exam")
const unifiedCocQuestions = [
  // ── Global Trend (Q1 - Q4) ──
  {
    id: "hu-2024-coc-01",
...
data.push(...unifiedCocQuestions);

fs.writeFileSync(EXAMS_PATH, JSON.stringify(data, null, 2), 'utf8');
console.log(`Added ${unifiedCocQuestions.length} unified COC questions. Total questions in database: ${data.length}`);
